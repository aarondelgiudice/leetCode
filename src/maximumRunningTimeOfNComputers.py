...
class Solution:
    def maxRunTime(self, n: int, batteries: List[int]) -> int:
        # left == min runtime limit -> 1 minute
        # right == max runtime limit -> int(sum(batteries) / n)
        left, right = 1, sum(batteries) // n 
        
        # binary search
        while left < right:
            # target == midpoint between left, right
        ...
        # return max potential runtime
        return left


if __name__ == "__main__":
    INPUTS = (
        # n batteries       expected
        (2, [3, 3, 3],      4),
        (2, [1, 1, 1, 1],   2),
    )

    for n, batteries, expected in INPUTS:
        actual = Solution().maxRunTime(n, batteries)
        assert actual == expected, f"{n=}, {batteries=}, {expected=}, {actual=}"
